package com.wishadesign.app.dtrac.util;

/**
 * Created by aksha on 12/10/2017.
 */

public class Config {

    public static final String BASE_URL = "https://www.vsstechnology.com/";

    public static final String LOGIN = "cp/cp-login.php";
    public static final String USER_DETAILS = "cp/api/get-cp-info.php";
    public static final String PROCESS = "cp/process.php";
    public static final String GET_AGENT_LIST = "cp/api/get-agent-list.php";
    public static final String GET_OUTLET_LIST = "cp/api/get-outlet-list.php";
    public static final String GET_ORDERS = "cp/api/get-order-list.php";
    public static final String GET_FIXED_ASSIGNMENT_LIST = "cp/api/get-fixed-assignments.php";
    public static final String FORGOT_PASSWORD = "cp/api/forgot-password.php";

    public static final String SIGNUP_CP_LINK = "http://www.vsstechnology.com/channel-partner.php";

}
